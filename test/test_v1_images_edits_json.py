...
class ImageEditsJsonApiTests(unittest.TestCase):
    ...
    def test_image_edit_accepts_remote_json_url(self):
        ...
        with mock.patch("api.image_inputs.requests.get", return_value=downloaded) as fetch:
            response = self.client.post(
                "/v1/images/edits",
                headers=AUTH_HEADERS,
    ...
    def test_image_edit_rejects_private_remote_url_without_fetching(self):
        with mock.patch("api.image_inputs.requests.get") as fetch:
            response = self.client.post(
                "/v1/images/edits",
                headers=AUTH_HEADERS,
                json={"prompt": "private URL", "image_url": "http://127.0.0.1/admin"},
            )
